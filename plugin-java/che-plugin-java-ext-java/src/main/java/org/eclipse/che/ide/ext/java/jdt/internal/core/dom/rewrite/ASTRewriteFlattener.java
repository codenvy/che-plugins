...
public class ASTRewriteFlattener extends ASTVisitor {
   ...
   /* package */static final int JLS2_INTERNAL = AST.JLS2;
    ...
    protected List<ASTNode> getChildList(ASTNode parent, StructuralPropertyDescriptor childProperty) {
        return (List<ASTNode>)getAttribute(parent, childProperty);
    }
    ...
    protected void visitList(ASTNode parent, StructuralPropertyDescriptor childProperty, String separator) {
        List<ASTNode> list = getChildList(parent, childProperty);
    ...
    public boolean visit(ArrayCreation node) {
        ...
        List<ASTNode> list = getChildList(node, ArrayCreation.DIMENSIONS_PROPERTY);
        for (int i = 0; i < list.size(); i++) {
            this.result.append('[');
            list.get(i).accept(this);
            this.result.append(']');
    ...
    public boolean visit(InfixExpression node) {
        ...
        List<ASTNode> list = getChildList(node, InfixExpression.EXTENDED_OPERANDS_PROPERTY);
        for (int i = 0; i < list.size(); i++) {
            this.result.append(operator);
            list.get(i).accept(this);
        }
    ...
    public boolean visit(Javadoc node) {
        this.result.append("/**"); //$NON-NLS-1$
        List<ASTNode> list = getChildList(node, Javadoc.TAGS_PROPERTY);
        for (int i = 0; i < list.size(); i++) {
            this.result.append("\n * "); //$NON-NLS-1$
            list.get(i).accept(this);
        }
    ...
    public boolean visit(SuperConstructorInvocation node) {
        ...
        if (node.getAST().apiLevel() >= AST.JLS3) {
            visitList(node, SuperConstructorInvocation.TYPE_ARGUMENTS_PROPERTY, String.valueOf(','), String.valueOf('<'),
                      String.valueOf('>'));
        }
    ...
    public boolean visit(TypeDeclaration node) {
        ...
        ChildPropertyDescriptor superClassProperty =
                (apiLevel == JLS2_INTERNAL) ? TypeDeclaration.SUPERCLASS_PROPERTY : TypeDeclaration.SUPERCLASS_TYPE_PROPERTY;
        ...
        ChildListPropertyDescriptor superInterfaceProperty =
                (apiLevel == JLS2_INTERNAL) ? TypeDeclaration.SUPER_INTERFACES_PROPERTY
    ...
    public boolean visit(TagElement node) {
        ...
        List<ASTNode> list = getChildList(node, TagElement.FRAGMENTS_PROPERTY);
        for (int i = 0; i < list.size(); i++) {
            if (i > 0 || tagName != null) {
                this.result.append(' ');
            }
    ...
    public boolean visit(EnumDeclaration node) {
        ...
        this.result.append(' ');
        visitList(node, EnumDeclaration.SUPER_INTERFACE_TYPES_PROPERTY, String.valueOf(','),
                  "implements ", Util.EMPTY_STRING); //$NON-NLS-1$
