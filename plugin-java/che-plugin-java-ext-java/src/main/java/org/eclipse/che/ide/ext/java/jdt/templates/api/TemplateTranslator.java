...
public class TemplateTranslator {
    /**
     * Regex pattern for identifier. Note: For historic reasons, this pattern <em>allows</em> numbers at the beginning of an
     * identifier.
     *
     * @since 3.7
     */
    private static final String IDENTIFIER = "(?:[a-zA-Z_$][a-zA-Z\\d_$]*)"; //$NON-NLS-1$
    ...
    /**
     * Precompiled regex pattern for valid dollar escapes (dollar literals and variables) and (invalid) single dollars.
     *
     * @since 3.3
     */
    private static final Pattern ESCAPE_PATTERN = Pattern.compile("\\$\\$|\\$\\{" + // $$|${                               //$NON-NLS-1$
                                                                  SPACES + "(" + IDENTIFIER + "?)" +
                                                                  // variable id group (1)               //$NON-NLS-1$ //$NON-NLS-2$
                                                                  SPACES + "(?:" + //$NON-NLS-1$
                                                                  ":" + //$NON-NLS-1$
                                                                  SPACES + "(" + QUALIFIED_NAME + ")" +
                                                                  // variable type group (2)        //$NON-NLS-1$ //$NON-NLS-2$
                                                                  SPACES + "(?:" + //$NON-NLS-1$
                                                                  "\\(" + // (                                    //$NON-NLS-1$
                                                                  SPACES + "((?:(?:" + ARGUMENT + ")" + SPACES + "," + SPACES + ")*(?:" +
                                                                  ARGUMENT + "))" +
                                                                  // arguments group (3) //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                                                                  //$NON-NLS-4$ //$NON-NLS-5$
                                                                  SPACES + "\\)" + // )                                    //$NON-NLS-1$
                                                                  ")?" + //$NON-NLS-1$
                                                                  SPACES + ")?" + //$NON-NLS-1$
    ...
    private final class VariableDescription {
        ...
        void mergeType(TemplateVariableType type) throws TemplateException {
            ...
            if (!type.equals(fType))
                fail("Template variable ''" + fName + "'' has incompatible types"); //$NON-NLS-1$
        }
    }
    ...
    /**
     * Used to ensure compatibility with subclasses overriding {@link #createVariable(String, String, int[])}.
     *
     * @since 3.3
     */
    ...
    /**
     * Returns an error message if an error occurred for the last translation, <code>null</code> otherwise.
     *
     * @return the error message if an error occurred during the most recent translation, <code>null</code> otherwise
     */
    ...
    /**
     * Translates a template to a <code>TemplateBuffer</code>. <code>null</code> is returned if there was an error.
    ...
    /**
     * Translates a template string to <code>TemplateBuffer</code>. <code>null</code> is returned if there was an error.
     * <code>getErrorMessage()</code> retrieves the associated error message.
     *
     * @param string
    ...
    private TemplateBuffer parse(String string) throws TemplateException {
        ...
        while (matcher.find()) {
            ...
            if ("$".equals(matcher.group())) { //$NON-NLS-1$
                fail("Template has incomplete variables. Type '$$' to enter the dollar character."); //$NON-NLS-1$
    ...
    /**
     * If there is no variable named <code>name</code>, a new variable with the given type, name and offset is created. If one
     * exists, the offset is added to the variable and the type is merged with the existing type.
     *
     * @param variables
    ...
    private TemplateVariable[] createVariables(Map variables) {
        ...
        for (Iterator it = variables.values().iterator(); it.hasNext(); idx++) {
            ...
            fCurrentType = type;
         /*
          * Call the deprecated version of createVariable. When not overridden, it will delegate to the new version using
          * fCurrentType.
          */
