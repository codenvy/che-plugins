...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.parser.AbstractCommentParser;
...
class DocCommentParser extends AbstractCommentParser {
    ...
    protected boolean parseTag(int previousPosition) throws InvalidInputException {
        ...
        if (this.scanner.currentCharacter != ' ' && !ScannerHelper.isWhitespace(this.scanner.currentCharacter)) {
            tagNameToken:
            while (token != TerminalTokens.TokenNameEOF && this.index < this.scanner.eofPosition) {
            ...
            case TerminalTokens.TokenNameIdentifier:
                switch (tagName[0]) {
                    ...
                    case 'l':
                        ...
                        break;
                    case 'v':
                        if (this.sourceLevel >= ClassFileConstants.JDK1_5 && length == TAG_VALUE_LENGTH
    ...
    protected boolean pushThrowName(Object typeRef) {
        ...
        throwsTag
                .setSourceRange(this.tagSourceStart, this.scanner.getCurrentTokenEndPosition() - this.tagSourceStart + 1);
