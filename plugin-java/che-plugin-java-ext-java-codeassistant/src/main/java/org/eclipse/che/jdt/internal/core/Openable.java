...
//                                            : null;
//        if(performanceStats != null) {
//            performanceStats.startRun(new String(cu.getFileName()) + " at [" + offset + "," + length + "]"); //$NON-NLS-1$
// $NON-NLS-2$ //$NON-NLS-3$
//        }
//
//        JavaProject project = (JavaProject)getJavaProject();
...
//        return requestor.getElements();
    ...
    protected void generateInfos(Object info, HashMap newElements, IProgressMonitor monitor) throws JavaModelException {
        ...
        if (JavaModelCache.VERBOSE) {
            ...
            System.out.println(
                    Thread.currentThread() + " OPENING " + element + " " + this.toStringWithAncestors()); //$NON-NLS-1$//$NON-NLS-2$
        }
