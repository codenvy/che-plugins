...
public class GitConfigurationCheckerTest {
    ...
    public static void init() throws Exception {
        Path targetDir = Paths.get(Thread.currentThread().getContextClassLoader().getResource(".").toURI()).getParent();
    ...
    public void testWhenNoGitconfigFile() throws Exception {
        ...
        Assert.assertTrue("New global .gitconfig file should be created in case it doesn't exists.",
                          Files.exists(globalGitconfigFilePath));
        Assert.assertTrue("New global .gitignore file should be created.",
                          Files.exists(gitignoreFilePath));
        Assert.assertEquals(excludesfilePropertyContent, new String(Files.readAllBytes(globalGitconfigFilePath)));
    ...
    public void testWhenNoExcludesfilePropertyInGitconfigFile() throws Exception {
        ...
        final String existingGitconfigFileContent = new String(Files.readAllBytes(globalGitconfigFilePath));
        checker.start();

        Assert.assertTrue(Files.exists(globalGitconfigFilePath));
        Assert.assertEquals("'core.excludesfile' property should be appended to the existing global .gitconfig file",
